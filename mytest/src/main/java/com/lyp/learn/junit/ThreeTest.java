...
public class ThreeTest {
   ...
   public void test00011(){
    ...
    public static LocalDate findCalSameWeekAndDay(LocalDate inputDate) {
        ...
        // 尝试在去年直接查找相同周数和星期几
        LocalDate lastYearSameWeek = findInYear(inputDate.getYear() - 1, inputWeek, inputDayOfWeek);
     ...
     * 在指定年份查找匹配的周数和星期几
     */
    private static LocalDate findInYear(int year, int week, DayOfWeek dayOfWeek) {
        try {
            LocalDate date = LocalDate.of(year, 1, 1)
                    .with(WeekFields.ISO.weekOfWeekBasedYear(), week)
    ...
    private static LocalDate getLastYearToday(LocalDate date) {
        ...
        // 处理2月29日情况
        if (monthDay.equals(MonthDay.of(Month.FEBRUARY, 29)) &&
                !Year.isLeap(lastYear)) {
