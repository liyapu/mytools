...
public class LocalDateTimeUtil {
    ...
    public static String format(TemporalAccessor time, String format) {
        final DateTimeFormatter formatter = StrUtil.isBlank(format) ? null : DateTimeFormatter.ofPattern(format);
     ...
     * @param text 日期字符串
     * @return LocalDateTime
     * @throws IllegalArgumentException 如果匹配不到合适的
     * @throws DateTimeParseException   如果转换出错
     */
    public static LocalDateTime parse(String text) {
        return DatePattern
                .getLocalDateTimePattern(text)
                .map(formatter -> parse(text, formatter))
                .orElseThrow(() -> {
                    String patterns = StrUtil.joinWith(";", DatePattern.getAllLocalDateTimePatternRegex()
                            .stream()
                            .map(Tuple2::v2)
                            .collect(Collectors.toList()));

                    return new IllegalArgumentException(StrUtil.format("text:{}不支持的日期格式,当前支持的日期格式有:{}",
                            text,
                            patterns));
                });
    }
