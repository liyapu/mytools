...
public class IPUtils{
	...
	static{
		try{
			...
			logger.error("初始化 IPSeeker 失败！", e);
		}
	}
...
class IPSeeker {
	...
	// 纯真IP数据库名
	private String IP_2_AREA_FILE = "/data.txt";
	...
	public IPSeeker() throws URISyntaxException {
		...
		b3 = new byte[3];
		try {
			ipFile = new RandomAccessFile(new File(this.getClass().getResource(IP_2_AREA_FILE).toURI()), "r");
	...
	public String getCountry(String ip) {
		String rawCountry = getCountry(Util.getIpByteArrayFromString(ip));
		if(provinceSet.contains(rawCountry.substring(0,2))){
			return "中国";
		}else{
			return rawCountry;
		}
